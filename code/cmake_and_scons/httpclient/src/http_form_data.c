...
int httpclient_formdata_addtext(http_client_data_t* client_data, char* content_disposition, char* content_type, char* name, char* data, int data_len)
{
    ...
    if((content_disposition == NULL) || (name == NULL) || (data == NULL) || (data_len == 0)) {
        printf("%s:%d invalid params", __func__, __LINE__);
        return -1;
    }

    if(strlen(data) > data_len) {
        printf("%s:%d invalid data_len %d strlen data %d", __func__, __LINE__, data_len, strlen(data));
        return -1;
    }

    if((data_info = found_formdata_info(client_data)) == NULL) {
        if((data_info = found_empty_formdata_info()) == NULL) {
            printf("%s:%d found no client_data info", __func__, __LINE__);
            return -1;
        }
    }
    ...
    if(data_info->is_used == 0) {
        ...
        if(data_info->form_data == NULL) {
            data_info->is_used = 0;
            printf("%s:%d form data malloc failed", __func__, __LINE__);
            return -1;
        }
    ...
    if( current->data == NULL) {
        ...
        printf("%s:%d data malloc failed", __func__, __LINE__);
        return -1;
    }
...
void httpclient_clear_form_data(http_client_data_t * client_data)
{
    ...
    if(data_info == NULL) {
        printf("No form data info found");
        return;
    }
    ...
    if(current != NULL) {
        ...
        printf("No form data in form data info");
    }
...
int httpclient_formdata_len(http_client_data_t *client_data)
{
    ...
    do {
        ...
        if(current->is_file == 1) {
            ...
            if(fd == NULL) {
                printf("%s: open file(%s) failed errno=%d", __func__, current->file_path, errno);
                return -1;
            }
...
int http_client_send_formdata(http_client_t *client, http_client_data_t *client_data)
{
...
#if CONFIG_HTTP_FILE_OPERATE
    {
        ...
        while(!feof(fd)) {
            ...
            if(ret <= 0) {
                printf("fread failed returned %d errno=%d", ret, errno);
                return -1;
            }
            ...
            if (ret > 0) {
                ...
                printf("Connection error (send returned %d) errno=%d", ret, errno);
                return HTTP_ECONN;
            }
