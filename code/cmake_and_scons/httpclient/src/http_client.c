...
static int http_client_parse_url(const char *url, char *scheme, size_t max_scheme_len,
    char *host, size_t max_host_len, int *port, char *path, size_t max_path_len)
{
    if (NULL == url) 
    {
    ...
    { /* including NULL-terminating char */
        printf("Scheme str is too small (%ld >= %ld)", max_scheme_len, host_ptr - scheme_ptr + 1);
        return HTTP_EPARSE;
    }
    ...
    {
        printf("Host str is too small (%ld >= %ld)\r\n", max_host_len, host_len + 1);
        return HTTP_EPARSE;
    }
    ...
    {
        printf("Path str is too small (%ld >= %d)\r\n", max_path_len, path_len + 1);
        return HTTP_EPARSE;
    }
...
HTTP_RESULT_CODE http_client_conn(http_client_t *client, const char *url)
{
    ...
    if (res != HTTP_SUCCESS) 
    {
        printf("http_client_parse_url returned %d\r\n", res);
    ...
    // printf("httpclient_conn() result:%d, client:%p", ret, client);
    return (HTTP_RESULT_CODE)ret;
}

/* 0 on success, err code on failure */
static int http_client_get_info(http_client_t *client, char *send_buf,
    int *send_index, char *buf, size_t buf_len)
{
    int ret ;
    int cp_len ;
...
static int http_client_send_header(http_client_t *client, const char *url, int method, http_client_data_t *client_data)
{
    ...
    printf("Trying to write %d bytes http header:\r\n%s", len, send_buf);
    ...
    if (ret > 0)
    {
        printf("Written %d bytes, socket = %d\r\n", ret, client->socket);
    } 
    else if ( ret == 0 ) 
    {
...
HTTP_RESULT_CODE http_client_send(http_client_t *client, const char *url, int method, http_client_data_t *client_data)
{
    ...
    // printf("httpclient_send() result:%d, client:%p", ret, client);
...
static int http_client_retrieve_content(http_client_t *client, char *data, int len, http_client_data_t *client_data)
{
    ...
    if (client_data->response_content_len == -1 && client_data->is_chunked == false) 
    {
        while(true)
        {
            ...
            if (max_len <= 0) 
            {
                printf("%s %d error max_len %d", __func__, __LINE__, max_len);
                return HTTP_EUNKOWN;
            }
            ...
            printf("in loop %s %d ret %d len %d\r\n", __func__, __LINE__, ret, len);
        }
    }
    ...
    while (true) 
    {
            ...
            do {
                ...
                { 
                    ...
                    if ( len < HTTP_CLIENT_CHUNK_SIZE ) 
                    {
                        ...
                        if (max_recv - len - 1 <= 0) 
                        {
                            printf("%s %d error max_len %d", __func__, __LINE__, max_recv - len - 1);
                            return HTTP_EUNKOWN;
                        }
                ...
                printf("in loop %s %d len %d ret %d\r\n", __func__, __LINE__, len, ret);
            }while (!foundCrlf);
            
            data[crlf_pos] = '\0';
            n = sscanf(data, "%lx", &readLen);/* chunk length */
        ...
        printf("Retrieving %ld bytes, len:%d\r\n", readLen, len);

        do {
            int ret;
            printf("readLen %ld, len:%d\r\n", readLen, len);
    ...
    while ( true ) 
    {
        ...
        if (crlf_ptr == NULL) 
        {
            if ( len < HTTP_CLIENT_CHUNK_SIZE - 1 ) 
            {
                ...
                {
                    printf("%s %d error max_len %d", __func__, __LINE__, HTTP_CLIENT_CHUNK_SIZE - len - 1);
                    return HTTP_EUNKOWN;
                }
                ...
                {
                    printf("in loop %s %d ret %d len %d", __func__, __LINE__, read_result, len);
                    continue;
                }
            } 
            else 
        ...
        if (colon_ptr) 
        {
            ...
            else if ((client->response_code >= 300 && client->response_code < 400) && (0 == strncasecmp(key_ptr, "Location", key_len))) 
            {
                ...
                {
                    printf("url is too large (%d >= %d)", value_len + 1, HTTP_CLIENT_MAX_URL_LEN);
                    return HTTP_EUNKOWN;
                }
...
HTTP_RESULT_CODE http_client_recv(http_client_t *client, http_client_data_t *client_data)
{
    ...
    if (client->socket < 0) 
    {
        printf("Invalid socket fd %d!", client->socket);
        return (HTTP_RESULT_CODE)ret;
    }
