...
int http_tcp_send_wrapper(http_client_t *client, const char *data, int length)
{
    ...
    while (written_len < length) 
    {
        ...
        {
            printf("Connection err ret=%d errno=%d\n", ret, errno);
...
int http_tcp_conn_wrapper(http_client_t *client, const char *host)
{
    ...
    {
        printf("getaddrinfo != 0, return EDNS");
        return HTTP_EDNS;
    }
...
int http_tcp_recv_wrapper(http_client_t *client, char *buf, int buflen, int timeout_ms, int *p_read_len)
{
    ...
    while (readLen < buflen) 
    {
        ...
        if (select_ret > 0) 
        {
            ...
            */
            ...
            if (ret < 0 ) 
            {
                ...
                {
                    printf("recv continue %d ret %d", err_record, ret);
                    continue;
                }
            } 
            else if (ret == 0) 
            {
                printf("recv return 0 disconnected");
                ret = HTTP_ECLSD;
            }
        }
        else if (select_ret == 0)
        {
            printf("select return 0 may disconnected");
            ret = HTTP_ECLSD;
        } 
        else 
        {
            printf("select return %d errno %d", select_ret, err_record);
            if (err_record == EINTR) 
            {
                continue;
            }
        ...
        {
            printf("Connection error (recv returned %d readLen:%d)", ret,readLen);
            *p_read_len = readLen;
            return HTTP_ECONN;
        }
    }
...
int http_ssl_recv_wrapper(http_client_t *client, char *buf, int buflen, int timeout_ms, int *p_read_len)
{
    ...
    while (readLen < buflen) {
        ...
        if (ret == 0) {
            ...
            printf("mbedtls_ssl_read, return:%d", ret);
...
int http_tcp_recv_file_wrapper(http_client_t *client, char *buf, int buflen, int timeout_ms, int *p_read_len, const char *filename)
{
    ...
    printf("http_get_sample: fd:%d  filename:%s --\n", fd, filename);
    if (fd < 0)
    {
        return ret;
    }
