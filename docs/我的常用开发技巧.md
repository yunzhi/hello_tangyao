...
8. ubuntu 扩容
   ...
   vgdisplay 查看FREE PE/size

   lvextend -L +50G /dev/mapper/ubuntu--vg-root 扩容50G

   resize2fs /dev/mapper/ubuntu--vg-root 刷新

   

   FYI[https://blog.csdn.net/Fly_1213/article/details/105142427]
...
| 目的                  | 操作                                       | 说明                                                         |
| --------------------- | ------------------------------------------ | ------------------------------------------------------------ |
| 立即释放磁盘          | 直接删除缓存目录                           | 关闭 VS Code 后删掉 `%LocalAppData%\Microsoft\vscode-cpptools\`（Win）或 `~/.cache/vscode-cpptools/`（Unix）。无副作用，只是下次打开时会重新生成缓存，首次 IntelliSense 稍慢。 |
| 限制缓存大小          | 设置 `"C_Cpp.intelliSenseCacheSize"`       | 在 `settings.json` 里加：`"C_Cpp.intelliSenseCacheSize": 512`（单位 MB，0 表示完全禁用缓存）。 |
| 把缓存搬到别处        | 设置 `"C_Cpp.intelliSenseCachePath"`       | 指向空间充足的磁盘或外置 SSD。                               |
| 定期自动清理          | 打开设置 `"C_Cpp.intelliSenseCacheRename"` | 设为 `true` 后，扩展会在同名缓存文件超过阈值时自动轮换，避免无限增长。 |
| 删除旧版本扩展        | 手动删或用命令                             | 在扩展目录里删多余 `ms-vscode.cpptools-*` 文件夹；或运行 `code --list-extensions --show-versions` 后用脚本清理旧版本（VS Code 1.86+ 开始会自动清理，但老版本不会）。 |
| 彻底不用 IntelliSense | 改用 Tag Parser 或 clangd                  | 把 `"C_Cpp.intelliSenseEngine"` 设成 `"Tag Parser"`（功能弱很多）；或直接卸载 cpptools，装 clangd 扩展 + clangd 服务器，磁盘占用小，体验也不错。 |
...
```
C:\Users\%USERNAME%\.lingma\index\meta\v4\index.db
```

删不掉就干掉 Lingma.exe 进程

修改.lingma位置：

```
C:\Users\%USERNAME%\.vscode\extensions\alibaba-cloud.tongyi-lingma-1.4.10\dist\extension.js

将(a.homedir(),".lingma")改成('D:\\',".lingma")即可；
```



一个windows查看硬盘文件大小的小软件 -- WizTree
...
推荐一个全局手势工具 [WGestures](http://www.yingdev.com/projects/wgestures) （可以置顶窗口）或者 [snipaste](https://zh.snipaste.com/) （很好用的截图工具，按下 `F1` 来开始截图，再按 `F3`，截图就在桌面置顶显示了），flux（屏幕亮度根据时间自动调节）
...
git pull --rebase 拉远程代码时对齐基线，这样提交记录上就没有“Merge branch ...”
...
```
git commit --amend --author="xx <xx@xxx.com>" --no-edit
```
...
```
git apply  --whitespace=fix xxx.patch 单纯的将修改合入代码中
```
...
#### 3.4.8  git导出差异文件及目录结构

1、 提取未提交的修改文件及目录

```
git status | grep modified | awk '{print $2}' > ../diff/list.txt
mkdir ../diff/diff_file
xargs -a ../diff/list.txt cp --parents -t ../diff/diff_file
```

2、 提取已提交记录中修改及增加的文件及目录

```
git log 查找提交记录中想要提取的提交记录ID
git log f3794bd30cb1d3f8e64f1177d3149f28cbc828e6(提交记录ID) -1 --name-only | grep '/' | awk '{print $1}' > ../diff/list.txt
mkdir ../diff/supplier
xargs -a ../diff/list.txt cp --parents -t ../diff/supplier
```



#### 3.4.9  git屏蔽git对于文件权限的检查
...
name = 'Eric'
percent = 20.111
print(f'Hello, my name is {name}, percent {percent:.1f}%')
...
> 输出结果：
>
> Hello, my name is Eric, percent 20.1%
>
> My name is Eric,
> age 18
>
...
list 中，下表从0开始， -1 获取最后一个元素，[:] 表示整个列表，[x,y]  表示从x下标开始到y下标的元素（不包含y）

-1 表示倒数第一个，-2 表示倒数第二个， [-3:-1] 表示从倒数第三个到倒数第一个之前的元素（不包含倒数第一个）
...
test_tuple = (3,4) #Python允许在list或tuple前面加一个`*`号，把list或tuple的元素变成可变参数传进去
print(calc(*test_tuple))
```
...
参数定义的顺序必须是：必选参数、默认参数、可变参数(*number)、命名关键字参数和关键字参数(\*\*kw)。
...
2. SSHFS-Win（基于 WinFsp 的 SSHFS 实现）

```
net use Z: \\sshfs.r\youruser@192.168.1.100\home\youruser\share
```
...
```
docker run --name t113 -it -d -p 3316:22 -v /data1:/data2 ubuntu_18.04_for_t113
```
...
-v /data1:/data2 : 将宿主机/data1目录挂载到docker容器下的/data2目录
...
```
app@localhost:~$ sudo docker ps
CONTAINER ID   IMAGE                   COMMAND       CREATED        STATUS        PORTS                                   NAMES
25d5d993e667   ubuntu_18.04_for_t113   "/bin/bash"   16 hours ago   Up 16 hours   0.0.0.0:3316->22/tcp, :::3316->22/tcp   t113
```
...
```
docker exec -it t113 /bin/bash
```



停止（优雅关闭，10 秒超时后强制 kill）
...
根据dockerfile创建docker

以 3506 的环境为例

1. 创建文件`dockerfile`，内容如下：

```
FROM ubuntu:22.04
MAINTAINER oneos "oneos@cmiot.chinamobile.com"
...
RUN apt install -y repo git ssh libssl-dev liblz4-tool lib32stdc++6 \
        ...
        subversion asciidoc w3m dblatex graphviz cpio \
        libparse-yapp-perl default-jre patchutils swig expect-dev u-boot-tools \
  clang-format astyle libncurses5-dev build-essential libgmp-dev libmpc-dev

RUN apt-get install -y --no-install-recommends \
        python2 python2-dev curl && \
...
RUN groupadd -g ${GID} firefly && \
      useradd -u ${UID} -g ${GID} -m -s /bin/bash firefly
RUN usermod -a -G sudo firefly
RUN echo 'firefly ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers.d/firefly

# USER firefly
WORKDIR /home/firefly
```

2. 在dockerfile 所在的目录执行下面的命令，创建docker image
...
干掉 Microsoft PC Manager Service 服务



### 3.14  tar相关

一行命令，只把  .tar.gz  里的 kernel-6.1 文件夹 解出来，其他内容全部忽略：

```
tar -xvf archive.tar.gz --wildcards '*/kernel-6.1/*' --strip-components=1
```
...
对于fm20s400，根据手册的意思，AMR0-3 寄存器中"0"表示 ACR0-3 中相应位置的位必须与消息标识符匹配，"1"表示屏蔽，即不需要与消息标识符匹配。 (这个掩码模式可能与一些cpu的用法有区别)
...
[CAN同步机制，你真的了解吗？ - 知乎](https://zhuanlan.zhihu.com/p/538642198)

[用示波器排查CAN的各种错误帧（1） - 知乎](https://zhuanlan.zhihu.com/p/684305455)

CAN 总线网络，需要在网络的两端各连接一个终端电阻，一般 CAN 主站端会内置一个终端电阻，所

以一般情况下只要在节点网络最末端连接一个终端电阻即可.

注：如果网络两端均正确连接了两个终端电阻，用外用表在断电情况下测量 CAN_H 和 CAN_L 的电阻，
